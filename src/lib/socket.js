...
import { NextResponse } from 'next/server';
import { getTenantContext } from '@/middleware/tenant'; // Fixed: Correct import path
import { getTenantDB } from '@/config/database'; // Fixed: Use existing getTenantDB
import CompanyAccountSchema from '@/models/schemas/CompanyAccount'; // Added: For dynamic model
import WebChatSessionSchema from '@/models/schemas/WebChatSession'; // Added: For dynamic model
import crypto from 'crypto';
...
export async function GET(request) {
  let tenantCtx;
  try {
    tenantCtx = await getTenantContext(request); // Fixed: Await and pass request
  } catch (error) {
    return NextResponse.json(
      { success: false, message: 'Unauthorized' },
      { status: 401 }
  ...
  try {
    const tenantDB = await getTenantDB(tenantCtx.tenantId); // Fixed: Use getTenantDB with tenantId
    const CompanyAccount = tenantDB.models.CompanyAccount || tenantDB.model('CompanyAccount', CompanyAccountSchema); // Fixed: Dynamic model
    ...
    // Get widget statistics
    const stats = await getWidgetStats(account.credentials.widgetId, tenantDB); // Fixed: Pass tenantDB
 ...
 * Get widget statistics
 */
async function getWidgetStats(widgetId, tenantDB) { // Fixed: Accept tenantDB param
  try {
    const WebChatSession = tenantDB.models.WebChatSession || tenantDB.model('WebChatSession', WebChatSessionSchema); // Fixed: Dynamic model

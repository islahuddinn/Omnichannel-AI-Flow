// src/components/forms/SMSSetupForm.jsx
'use client';
...
export default function SMSSetupForm({ 
    ...
    if (initialData) {
      ...
      setFormData(prev => ({
        ...
        credentials: {
          ...
          senderId: initialData.credentials?.senderId || '',
        },
        aiPrompts: {
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      case 1:
        ...
        if (!isEdit && (!formData.credentials.apiKey || formData.credentials.apiKey.trim() === '')) {
  ...
  const steps = [
    { number: 1, title: 'SMS Provider', subtitle: 'Channel Details' },
  ...
  const isStepCompleted = (stepNumber) => {
    switch (stepNumber) {
      case 1:
        return !!(formData.name && formData.identifier && formData.departmentIds?.length > 0 && (isEdit || formData.credentials.apiKey));
      case 2:
      ...
      <form onSubmit={handleFormSubmit} className="space-y-6" onKeyDown={(e) => {
        ...
        {currentStep === 1 && (
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
            <CardHeader>
              <CardTitle className="text-gray-900 dark:text-gray-100">SMS Provider</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="apiKey" className="text-gray-900 dark:text-gray-100">
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(e) => {
                      handleChange('credentials.apiKey', e.target.value);
                      if (errors.apiKey) setErrors(prev => ({ ...prev, apiKey: '' }));
                    }}
                    placeholder={isEdit ? "Leave blank to keep existing" : "Enter your SMS API key"}
                    required={!isEdit}
                    className={`pr-10 bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-gray-100 ${
                      errors.apiKey ? 'border-red-500 dark:border-red-500' : ''
                    }`}
                  />
                  <button
                    type="button"
              ...
              <div className="space-y-2">
                <Label htmlFor="senderId" className="text-gray-900 dark:text-gray-100">Sender ID</Label>
                <Input
                  id="senderId"
                  value={formData.credentials.senderId}
                  onChange={(e) => handleChange('credentials.senderId', e.target.value)}
                  placeholder="Optional sender identifier"
          ...
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
            <CardHeader>
              ...
              <CardDescription className="text-gray-600 dark:text-gray-400">
                Configure AI responses for SMS. These prompts guide the AI bot's behavior and responses.
              </CardDescription>
            </CardHeader>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-2">
            ...
            {currentStep < 2 ? (
              ...
              >
                {isLoading ? (isEdit ? 'Updating...' : 'Connecting...') : (isEdit ? 'Update SMS' : 'Connect SMS')}

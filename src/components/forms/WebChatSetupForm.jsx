// src/components/forms/WebChatSetupForm.jsx
'use client';
...
import { User, ArrowLeft, ArrowRight, Lightbulb } from 'lucide-react';
import DepartmentSelector from './DepartmentSelector';

export default function WebChatSetupForm({ 
  onSubmit, 
  isLoading, 
  onCancel, 
  initialData, 
  ...
  const [formData, setFormData] = useState({
    ...
    identifier: '',
    departmentIds: [],
    credentials: { widgetId: '', publicKey: '' },
    aiPrompts: {
      customerPrompt: '',
      handymanPrompt: ''
    }
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData(prev => ({
        ...
        credentials: {
          widgetId: initialData.credentials?.widgetId || '',
          publicKey: initialData.credentials?.publicKey || '',
        },
        aiPrompts: {
      ...
      // Only generate new IDs for new widgets
      setFormData(prev => ({
        ...prev,
        identifier: `widget-${Date.now()}`,
        credentials: { ...prev.credentials, widgetId: `widget-${Date.now()}` }
      }));
    }
  }, [initialData, isEdit]);
  ...
  const handleCreateClick = async (e) => {
    ...
    if (!formData.name || formData.name.trim() === '') {
      validationErrors.name = 'Widget name is required';
    }
  ...
  const steps = [
    { number: 1, title: 'WebChat Widget', subtitle: 'Channel Details' },
  ...
  const isStepCompleted = (stepNumber) => {
    switch (stepNumber) {
      case 1:
        return !!(formData.name && formData.departmentIds?.length > 0);
      case 2:
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleFormSubmit} className="space-y-6" onKeyDown={(e) => {
        ...
        {/* Step 1: WebChat Widget */}
        {currentStep === 1 && (
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
            <CardHeader>
              <CardTitle className="text-gray-900 dark:text-gray-100">WebChat Widget</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="name" className="text-gray-900 dark:text-gray-100">Widget Name *</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="eg- Support WebChat"
                    required
              ...
              <div className="space-y-2">
                <Label htmlFor="widgetId" className="text-gray-900 dark:text-gray-100">Widget ID</Label>
                <Input
                  id="widgetId"
                  value={formData.credentials.widgetId}
                  onChange={(e) => handleChange('credentials.widgetId', e.target.value)}
                  disabled={!isEdit}
                  placeholder="Unique widget identifier"
                  className="bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
                />
                {!isEdit && (
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Widget ID will be generated automatically
                  </p>
                )}
              </div>
          ...
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
            <CardHeader>
              ...
              <CardDescription className="text-gray-600 dark:text-gray-400">
                Configure AI responses for WebChat. These prompts guide the AI bot's behavior and responses.
              </CardDescription>
            </CardHeader>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-2">
            ...
            {currentStep < 2 ? (
              ...
              >
                {isLoading ? (isEdit ? 'Updating...' : 'Creating...') : (isEdit ? 'Update Widget' : 'Create Widget')}

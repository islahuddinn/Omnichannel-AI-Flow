// src/components/forms/InstagramSetupForm.jsx
'use client';
...
export default function InstagramSetupForm({ 
  onSubmit, 
  isLoading, 
  onCancel, 
  initialData, 
  ...
  const [formData, setFormData] = useState({
    ...
    identifier: '',
    departmentIds: [],
    credentials: { instagramId: '', accessToken: '' },
    aiPrompts: {
      customerPrompt: '',
      handymanPrompt: ''
    }
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData(prev => ({
        ...
        credentials: {
          instagramId: initialData.credentials?.instagramId || '',
    ...
    switch (step) {
      case 1:
        ...
        if (!formData.identifier || formData.identifier.trim() === '') {
          newErrors.identifier = 'Instagram Business ID is required';
        }
        if (!formData.departmentIds || formData.departmentIds.length === 0) {
          newErrors.departmentIds = 'Please select at least one department';
        }
        if (!isEdit && (!formData.credentials.accessToken || formData.credentials.accessToken.trim() === '')) {
  ...
  const handleConnectClick = async (e) => {
    ...
    if (!formData.identifier || formData.identifier.trim() === '') {
      validationErrors.identifier = 'Instagram Business ID is required';
    }
  ...
  const steps = [
    { number: 1, title: 'Instagram Business', subtitle: 'Channel Details' },
  ...
  const isStepCompleted = (stepNumber) => {
    switch (stepNumber) {
      case 1:
        return !!(formData.name && formData.identifier && formData.departmentIds?.length > 0 && (isEdit || formData.credentials.accessToken));
      case 2:
      ...
      <form onSubmit={handleFormSubmit} className="space-y-6" onKeyDown={(e) => {
        ...
        {currentStep === 1 && (
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
            <CardHeader>
              <CardTitle className="text-gray-900 dark:text-gray-100">Instagram Business Account</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="eg- Support Instagram"
                    required
              ...
              <div className="space-y-2">
                <Label htmlFor="identifier" className="text-gray-900 dark:text-gray-100">Instagram Business ID *</Label>
                <Input
                  id="identifier"
              ...
              <div className="space-y-2">
                <Label htmlFor="accessToken" className="text-gray-900 dark:text-gray-100">
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(e) => {
                      handleChange('credentials.accessToken', e.target.value);
                      if (errors.accessToken) setErrors(prev => ({ ...prev, accessToken: '' }));
                    }}
                    placeholder={isEdit ? "Leave blank to keep existing" : "Enter your Instagram access token"}
                    required={!isEdit}
                    className={`pr-10 bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-gray-100 ${
                      errors.accessToken ? 'border-red-500 dark:border-red-500' : ''
                    }`}
                  />
                  <button
                    type="button"
                ...
                {errors.accessToken && (
                  <p className="text-sm text-red-600 dark:text-red-400">{errors.accessToken}</p>
                )}
                {isEdit && !errors.accessToken && (
          ...
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
            <CardHeader>
              ...
              <CardDescription className="text-gray-600 dark:text-gray-400">
                Configure AI responses for Instagram. These prompts guide the AI bot's behavior and responses.
              </CardDescription>
            </CardHeader>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-2">
            ...
            {currentStep < 2 ? (
              ...
              >
                {isLoading ? (isEdit ? 'Updating...' : 'Connecting...') : (isEdit ? 'Update Instagram' : 'Connect Instagram')}

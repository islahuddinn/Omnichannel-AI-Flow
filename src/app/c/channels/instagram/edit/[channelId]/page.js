// src/app/company-admin/admin/channels/instagram/edit/[channelId]/page.js
'use client';
...
export default function InstagramEditPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success('Instagram channel and AI prompts updated successfully');
        setTimeout(() => {
    ...
    onError: (error) => {
      console.error('Update mutation error:', error);
      toast.error(error.response?.data?.error || 'Failed to update Instagram channel');
    }
  });
  ...
  const customerPrompt = prompts.find(p => p.moduleIdDescription === 'instagram-customer-prompt');
  const handymanPrompt = prompts.find(p => p.moduleIdDescription === 'instagram-handyman-prompt');
  ...
  const initialFormData = {
    ...
    credentials: {
      instagramId: channel.credentials?.instagramId || '',
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">
            Edit Instagram Channel
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Update your Instagram Business account settings
          </p>
        </div>
      </div>

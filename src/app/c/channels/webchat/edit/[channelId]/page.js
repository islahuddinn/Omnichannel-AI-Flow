// src/app/company-admin/admin/channels/webchat/edit/[channelId]/page.js
'use client';
...
export default function WebChatEditPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success('WebChat widget and AI prompts updated successfully');
        setTimeout(() => {
    ...
    onError: (error) => {
      console.error('Update mutation error:', error);
      toast.error(error.response?.data?.error || 'Failed to update WebChat widget');
    }
  });
  ...
  const customerPrompt = prompts.find(p => p.moduleIdDescription === 'webchat-customer-prompt');
  const handymanPrompt = prompts.find(p => p.moduleIdDescription === 'webchat-handyman-prompt');
  ...
  const initialFormData = {
    ...
    credentials: {
      widgetId: channel.credentials?.widgetId || '',
      publicKey: channel.credentials?.publicKey || '',
    },
    aiPrompts: {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Update your WebChat widget settings
          </p>
        </div>
      </div>

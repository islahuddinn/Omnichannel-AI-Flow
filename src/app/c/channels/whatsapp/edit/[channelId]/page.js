// src/app/company-admin/admin/channels/whatsapp/edit/[channelId]/page.js
'use client';
...
export default function WhatsAppEditPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      try {
        ...
        toast.success('WhatsApp channel and AI prompts updated successfully');
        setTimeout(() => {
    ...
    onError: (error) => {
      console.error('Update mutation error:', error);
      toast.error(error.response?.data?.error || 'Failed to update WhatsApp channel');
    }
  });
  ...
  const customerPrompt = prompts.find(p => p.moduleIdDescription === 'whatsapp-customer-prompt');
  const handymanPrompt = prompts.find(p => p.moduleIdDescription === 'whatsapp-handyman-prompt');
  ...
  const initialFormData = {
    ...
    credentials: {
      ...
      phoneNumberId: channel.credentials?.phoneNumberId || '',
    },
    aiPrompts: {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Update your WhatsApp Business account settings
          </p>
        </div>
      </div>

// src/app/company-admin/admin/channels/email/edit/[channelId]/page.js
'use client';
...
export default function EmailEditPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success('Email channel and AI prompts updated successfully');
        setTimeout(() => {
    ...
    onError: (error) => {
      console.error('Update mutation error:', error);
      toast.error(error.response?.data?.error || 'Failed to update Email channel');
    }
  });
  ...
  const customerPrompt = prompts.find(p => p.moduleIdDescription === 'email-customer-prompt');
  const handymanPrompt = prompts.find(p => p.moduleIdDescription === 'email-handyman-prompt');
  ...
  const initialFormData = {
    ...
    credentials: {
      ...
      smtpPort: channel.credentials?.smtpPort || '587',
      smtpUser: channel.credentials?.smtpUser || '',
      smtpPass: channel.credentials?.smtpPass || '',
      imapHost: channel.credentials?.imapHost || '',
      imapPort: channel.credentials?.imapPort || '993',
    },
    aiPrompts: {

// src/app/company-admin/admin/channels/facebook/edit/[channelId]/page.js
'use client';
...
export default function FacebookEditPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success('Facebook channel and AI prompts updated successfully');
        setTimeout(() => {
    ...
    onError: (error) => {
      console.error('Update mutation error:', error);
      toast.error(error.response?.data?.error || 'Failed to update Facebook channel');
    }
  });
  ...
  const customerPrompt = prompts.find(p => p.moduleIdDescription === 'facebook-customer-prompt');
  const handymanPrompt = prompts.find(p => p.moduleIdDescription === 'facebook-handyman-prompt');
